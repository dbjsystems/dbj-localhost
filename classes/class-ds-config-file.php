...
/**
 * DesktopServer ConfigFile class allows for easy querying and updating of popular
 ...
 * @since 4.0.0
 */
if ( class_exists( 'DS_ConfigFile' ) ) return;
class DS_ConfigFile {
	// *** Private
	...
	function comment() {
		...
		$line = new GString( $this->block[$this->last_key_line] );
		if ( false === $this->is_commented() ) {
			$this->block[$this->last_key_line] = $this->comment_chars . (string) $line;
		}
	}
	...
	function uncomment() {
		...
		if ( true === $this->is_commented() ) {
			$this->block[$this->last_key_line] = (string) $line->delLeftMost( $this->comment_chars );
		}
	}
	...
	function get() {
		...
		return (string) $s->delLeftMost( $pre )->getLeftMost( $app );
	}
